...
# In our preliminary example, we want to check the influence of cases on business restrictions (be careful: This is only an example to illustrate the stats and does not make a lot of sense)
...
cases$ratio <- cases$cases/cases$World

cases<-cases[,-1]

test<- left_join(businessRestrictFull,cases, by=c("country","date_announced"))

didreg = lm(numBusinessRestrictions ~ ratio + time + I(ratio * time), data = test)
